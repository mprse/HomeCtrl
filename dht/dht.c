...
static uint8_t pio_prog_offset  = 0;
static uint8_t pio_sm = 0;
static uint8_t dma_chan = 0;
...
void dht_init(dht_t *dht, dht_model_t model, PIO pio, uint8_t data_pin, bool pull_up) {
    ...
    dht->pio = pio;
    if (init_done == 0) {
        pio_prog_offset = pio_add_program(pio, &dht_program);
        pio_sm = pio_claim_unused_sm(pio, true /* required */);
        dma_chan = dma_claim_unused_channel(true /* required */);
        init_done = 1;
    } 
    dht->pio_program_offset = pio_prog_offset;
    dht->sm = pio_sm;
    dht->dma_chan = dma_chan;
